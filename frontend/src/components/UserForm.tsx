...
const UserSchema = Yup.object().shape({
  ...
  birthday: Yup.date()
    ...
    .nullable(),
  status: Yup.number()
    .oneOf([UserStatus.ACTIVE, UserStatus.INACTIVE], 'Статус должен быть 10 (Активный) или 20 (Неактивный)')
...
const UserForm: React.FC<UserFormProps> = ({ user, onSuccess, onCancel }) => {
  ...
  const formik = useFormik<UserFormValues>({
    ...
    onSubmit: (values) => {
      const formattedValues: CreateUserData | UpdateUserData = {
      ...
      if (isEditing && user) {
        updateMutation.mutate({ id: user.id, userData: formattedValues });
      } else {
        createMutation.mutate(formattedValues as CreateUserData);
      }
    },
  });
  ...
  const getStatusLabel = (status: UserStatus): string => {
    return status === UserStatus.ACTIVE ? 'Активный' : 'Неактивный';
  };
    ...
    <div className="user-form">
      ...
      <form className="form" onSubmit={formik.handleSubmit}>
        ...
        <div className="form-group">
          ...
          >
            <option value={UserStatus.ACTIVE}>Активный</option>
            <option value={UserStatus.INACTIVE}>Неактивный</option>
          </select>
